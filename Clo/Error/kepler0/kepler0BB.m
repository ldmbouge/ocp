...
id<ORRational> (^kepler0Error)(NSMutableArray* arrayValue, NSMutableArray* arrayError) = ^(NSMutableArray* arrayValue, NSMutableArray* arrayError){
...
void kepler0_d_c_3B(int search, int argc, const char * argv[]) {
   @autoreleasepool {
      /* Creation of model */
      id<ORModel> mdl = [ORFactory createModel];
      ...
      /* Declaration of constraints */
      //(x2*x5) + (x3*x6) - (x2*x3) - (x5*x6) + (x1* (-x1+x2+x3-x4+x5+x6))
      [g add:[z set: [[[[[x2 mul: x5] plus: [x3 mul: x6]] sub: [x2 mul: x3]] sub: [x5 mul: x6]] plus: [x1 mul: [[[[[[x1 minus] plus: x2] plus: x3] sub: x4] plus: x5] plus: x6]]]]];
