...
id<ORRational> (^doppler2Error)(NSMutableArray* arrayValue, NSMutableArray* arrayError) = ^(NSMutableArray* arrayValue, NSMutableArray* arrayError){
         ...
         if (search)
            [cp branchAndBoundSearchD:vars out:ezAbs do:^(ORUInt i, id<ORDisabledVarArray> x) {
               [cp floatSplit:i withVars:x];
                              ...
                              compute:doppler2Error];
      }];
   }
}

void doppler2_d_c(int search, int argc, const char * argv[]) {
   @autoreleasepool {
      /* Creation of model */
      id<ORModel> mdl = [ORFactory createModel];
      
      /* Declaration of model variables */
      id<ORDoubleVar> u = [ORFactory doubleInputVar:mdl low:-125.0 up:125.0 name:@"u"];
      id<ORDoubleVar> v = [ORFactory doubleInputVar:mdl low:15.0 up:25000.0 name:@"v"];
      id<ORDoubleVar> t = [ORFactory doubleInputVar:mdl low:-40.0 up:60.0 name:@"t"];
...
void doppler2_d_c_3B(int search, int argc, const char * argv[]) {
   @autoreleasepool {
      /* Creation of model */
      id<ORModel> mdl = [ORFactory createModel];
...
int main(int argc, const char * argv[]) {
   ...
   //doppler2_d_c(1, argc, argv);
   doppler2_d_c_3B(1, argc, argv);
   return 0;
}
