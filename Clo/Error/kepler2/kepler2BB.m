...
id<ORRational> (^kepler2Error)(NSMutableArray* arrayValue, NSMutableArray* arrayError) = ^(NSMutableArray* arrayValue, NSMutableArray* arrayError){
...
void kepler2_d_c_3B(int search, int argc, const char * argv[]) {
   @autoreleasepool {
      /* Creation of model */
      id<ORModel> mdl = [ORFactory createModel];
      ...
      /* Declaration of constraints */
      // x1*x4(-x1+x2+x3-x4+x5+x6) + x2*x5(x1-x2+x3+x4-x5+x6) + x3*x6(x1+x2-x3+x4+x5-x6) - x2*x3*x4 - x1*x3*x5 - x1*x2*x6 - x4*x5*x6
      [g add:[z set: [[[[[[[[x1 mul: x4] mul: [[[[[[x1 minus] plus: x2] plus: x3] sub: x4] plus: x5] plus: x6] ] plus: [[x2 mul: x5] mul:[[[[[x1 sub: x2] plus: x3] plus: x4] sub: x5] plus: x6]]] plus: [[x3 mul: x6] mul: [[[[[x1 plus: x2] sub: x3] plus: x4] plus: x5] sub: x6]]] sub: [[x2 mul: x3] mul: x4]] sub: [[x1 mul: x3] mul: x5]] sub: [[x1 mul: x2] mul: x6]] sub: [[x4 mul: x5] mul: x6]]]];
