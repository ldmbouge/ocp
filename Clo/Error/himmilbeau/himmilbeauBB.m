...
void himmilbeau_d_c_3B(int search, int argc, const char * argv[]) {
   @autoreleasepool {
      /* Creation of model */
      id<ORModel> mdl = [ORFactory createModel];
      ...
      /* Declaration of constraints */
      //   res = (x1*x1 + x2 - 11)* (x1*x1 + x2 - 11) + (x1 + x2*x2 - 7)*(x1 + x2*x2 - 7);
      [g add:[z set:[[[[[x1 mul: x1] plus: x2] sub: @(11.0)] mul: [[[x1 mul: x1] plus: x2] sub: @(11.0)]] plus: [[[x1 plus: [x2 mul: x2]] sub: @(7.0)] mul:[[x1 plus: [x2 mul: x2]] sub: @(7.0)]]]]];
