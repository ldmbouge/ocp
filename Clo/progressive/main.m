...
////               [mdl add: [ORFactory multiknapsackOne:mdl item:All(mdl,ORIntVar, g, Guests, [boat at: g :p]) itemSize: crew bin: h capacity: [cap at: h]]];
...
//            
//            while (it < 1000000) {
//               ORInt old = [ls weightedViolations];
...
//               }];
////              printf(".");
//               //printf("(%d)",[ls weightedViolations]);
//               if ([ls getTrueViolations] < best) {
//                  best = [ls getTrueViolations];
////                  printf("[%d]",best);
//                  for(ORInt i = 0; i < nbv; i++)
//                     [bestSolution set: [ls intValue: x[i]] at: i];
//               }
//               if ([ls weightedViolations] < weightedBest) {
//                  stable = 0;
//                  weightedBest = [ls weightedViolations];
////                  printf("(%d/%d)",[ls weightedViolations],[ls violations]);
//                  for(ORInt i = 0; i < nbv; i++)
...
//                  stable++;
//               if ([ls isTrue])
////               if ([ls getTrueViolations] == 0)
//                  break;
////              printf("(%d)",[ls violations]);
////               for(ORInt p = Periods.low; p <= Periods.up; p++) {
////                  NSMutableString* line = [[NSMutableString alloc] initWithCapacity:64];
////                  [line appendFormat:@"p=%2d : ",p];
////                  for(ORInt g = Guests.low; g <= Guests.up; g++) {
...
//                  //               }
//
////               if ([ls weightedViolations] == 0 || it%10000 == 0) {
//                  stable = 0;
////                  for(ORInt i = 0; i < nbv; i++)
////                     [ls label: x[i] with: [weightedBestSolution at: i]] ;
//                  weightedBest = 2000000;
////                  printf("Multipliers\n");
...
//         
// 
//         struct ORResult r = REPORT([ls violations], it, 0, 0);
//         return r;
//      }];
//   }
//   return 0;
...
//                         assert(old + delta == violations);
               ...
               //              printf(".");
               //printf("(%d)",[ls weightedViolations]);
               if ([ls unweightedViolations] < best) {
                  best = [ls unweightedViolations];
//                                   printf("[%d]",best);
                  for(ORInt i = 0; i < nbv; i++)
                     [bestSolution set: [ls intValue: x[i]] at: i];
               }
