...
void doppler3_d(int search, int argc, const char * argv[]) {
   @autoreleasepool {
      ...
      id<ORDoubleRange> r0 = [ORFactory doubleRange:mdl low:-30.0 up:120.0];
      id<ORDoubleRange> r1 = [ORFactory doubleRange:mdl low:320.0 up:20300.0];
      id<ORDoubleRange> r2 = [ORFactory doubleRange:mdl low:-50.0 up:30.0];
      ...
      [cp solve:^{
         ...
         printDvar("t1", t1);
         printDvar("z", z);
         if (search) check_it_doppler3_d(getDmin(u), getDmin(v), getDmin(t), getDmin(t1), getDmin(z), getDminErr(z));
      }];
   }
}
