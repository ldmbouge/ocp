...
int main(int argc, const char * argv[])
{
    ...
    id<ORIntMatrix> t = [ORFactory intMatrix: master range: I : J with: ^ORInt(ORInt i, ORInt j) { return [distr next]; }];
    ...
    id<ORRunnable> benders = [ORFactory logicBenders: ip slave: ^id<ORConstraintSet>(id<ORSolution> sol) {
        id<ORConstraintSet> cuts = [ORFactory createConstraintSet];
        for (ORInt j = 0; j < n; j++) {
            id<ORIntSet> Ij = [ORFactory collect: master range: I suchThat: ^ORBool(ORInt i) {
                return [sol intValue: [x at: i : j]] == 1; } of: ^ORInt(ORInt e) { return e; } ];
            ...
            ORInt numVehj = [sol intValue: [numVeh at: j]];
            NSLog(@"FFD vs numVehj : %i %i", numVehFFD, numVehj);
            if(numVehFFD > numVehj) {
                // Run subproblems
                for(ORInt numVehBinPacking = numVehj; numVehBinPacking < numVehFFD; numVehBinPacking++) {
                    ...
                    id<ORIntVarArray> truck = [ORFactory intVarArray: subproblem range: RANGE(subproblem, 0, (ORInt)[dist count]-1) domain: RANGE(subproblem, 0, numVehj-1)];
                    NSLog(@"truck: %@ dist: %@ load: %@", [[truck range] description], [[dist range] description], [[load range] description]);
                    
                    [subproblem add: [ORFactory packing: subproblem item: truck itemSize: dist load: load]];
    ...
    [benders run];
    NSLog(@"best bound: %f", [benders bestBound]);
