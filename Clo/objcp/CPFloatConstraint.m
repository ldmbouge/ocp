...
-(id)initCPFloatSquareBC:(CPFloatVarI*)z equalSquare:(CPFloatVarI*)x
{
...
-(void)propagate
{
   ...
   do {
      ...
      if ([_x bound]) {
         ...
         if (ORISurePositive(zb))
            xs = [_x updateInterval:ORIPSqrt(zb)];
         else if (ORISureNegative(zb))
...
-(void)propagate
{
   ...
   do {
      ...
      for(ORInt i=_x.low;i <= _x.up;i++) {
         ...
         ORInterval TMP = ORISub(S, ORIMul(xii, ci > 0 ? createORI1(ci) : ORISwap(createORI1(ci))));
...
-(NSString*)description
{
   return [NSMutableString stringWithFormat:@"<CPFloatEquationBC:%02d %@ %@ == %f>",_name,_x,_coefs,_c];
}
@end
