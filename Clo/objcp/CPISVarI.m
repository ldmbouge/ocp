...
static void deallocNetwork(CPISVNetwork* net)
{
   ...
   freeList(net->_changeEvt[0]._val);
   freeList(net->_reqEvt[0]._val);
   freeList(net->_excEvt[0]._val);
}
...
{
   __block ORUInt d = 0;
   [_net._bindEvt[0]._val scanCstrWithBlock:^(CPCoreConstraint* cstr)   { d += [cstr nbVars] - 1;}];
   [_net._changeEvt[0]._val scanCstrWithBlock:^(CPCoreConstraint* cstr) { d += [cstr nbVars] - 1;}];
   [_net._reqEvt[0]._val scanCstrWithBlock:^(CPCoreConstraint* cstr)    { d += [cstr nbVars] - 1;}];
   [_net._excEvt[0]._val scanCstrWithBlock:^(CPCoreConstraint* cstr)    { d += [cstr nbVars] - 1;}];
   return d;
}
...
{
   [_engine scheduleValueClosure:[CPValueLossEvent newValueLoss:v notify:_net._reqEvt[0]._val]];
}
-(void)excludeEvt:(ORInt)v
{
   [_engine scheduleValueClosure:[CPValueLossEvent newValueLoss:v notify:_net._excEvt[0]._val]];
}
-(void)bindEvt
{
   id<CPClosureList> mList[2] = {_net._bindEvt[0]._val,NULL};
   scheduleClosures(_engine,mList);
}
-(void)changeEvt
{
   id<CPClosureList> mList[2] = {_net._changeEvt[0]._val,NULL};
   scheduleClosures(_engine,mList);
}
