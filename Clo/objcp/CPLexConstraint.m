...
@implementation CPLexConstraint {
   ...
   CPIntVarBase**   _xa; // 0-based version of _x
   CPIntVarBase**   _ya; // 0-based version of _y
...
-(ORStatus) post
{
   ...
   _sz = [_x count];
   _xa= malloc(sizeof(CPIntVarBase*)*_sz);
   _ya= malloc(sizeof(CPIntVarBase*)*_sz);
   for(ORInt k=[_x low]; k<= [_x up];k++) _xa[k - [_x low]] = (CPIntVarBase*)_x[k];
   for(ORInt k=[_y low]; k<= [_y up];k++) _ya[k - [_y low]] = (CPIntVarBase*)_y[k];
   ORLong up = _sz - 1;
   ORInt  i = 0;
   while (i <= up && LEX_XEQ_GEQY(i)) { // STATE 1 (x_i = y_i OR x_i >= y_i)
      ORInt xi = minDom(_xa[i]);
