...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        ...
        [args measure:^struct ORResult(){
            ...
            id<ORFloatVarArray> x = [ORFactory floatVarArray:model range:RANGE(model, 0, 16)];
            id<ORFloatVar> res = [ORFactory floatVar:model];
            
            [model add:[res eq:[[[x[0] sub:
                                  [[x[1] mul:[x[2] mul:x[3]]] div:@(6.0f)]] plus:
                                 [[x[4] mul:[x[5] mul:[x[6] mul:[x[7] mul:x[8]]]]] div:@(120.0f)]] sub:
                                [[x[9] mul:[x[10] mul:[x[11] mul:[x[12] mul:[x[13] mul:[x[14] mul:x[15]]]]]]] div:@(5040.0f)]]]];
