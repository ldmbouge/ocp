...
static void initThetaLambdaTreeWithLst(CPDisjunctive * disj, const ORUInt * idx_map_lct, ThetaTree * theta, LambdaTree * lambda, const ORUInt tsize) {
    // Inserting all tasks into the Theta tree
    for (ORUInt i = 0; i < disj->_size; i++) {
...
static void updateBounds(CPDisjunctive * disj)
{
    for (ORInt i = 0; i < disj->_size; i++) {
        if (disj->_new_est[i] > disj->_est[i]) {
            [disj->_start0[i] updateMin: disj->_new_est[i]];
            if (disj->_idempotent) disj->_est[i] = disj->_new_est[i];
        }
        if (disj->_new_lct[i] < disj->_lct[i]) {
            [disj->_start0[i] updateMax: disj->_new_lct[i] - disj->_dur_min[i]];
...
//  Space: O(n)
//
static void ef_overload_check_vilim(CPDisjunctive * disj, const ORUInt * idx_map_est, ThetaTree * theta, const ORUInt tsize)
{
    initThetaTree(theta, tsize, MININT);
    // Iteration in non-decreasing order of the latest completion time
    for (ORInt tt = 0; tt < disj->_size; tt++) {
...
//      of zero!
//
static void dprec_filter_est_and_lct_vilim(CPDisjunctive * disj, const ORUInt * idx_map_est, const ORUInt * idx_map_lct, ThetaTree * theta, const ORUInt tsize, bool * update)
{
    dprec_filter_est_vilim(disj, idx_map_est, theta, tsize, update);
    dprec_filter_lct_vilim(disj, idx_map_lct, theta, tsize, update);
    if (update) updateBounds(disj);
}

static void dprec_filter_est_vilim(CPDisjunctive * disj, const ORUInt * idx_map_est, ThetaTree * theta, const ORUInt tsize, bool * update)
{
    ...
    for (ORInt ii = 0; ii < disj->_size; ii++) {
        const ORInt i = disj->_task_id_ect[ii];
        // Inner loop:
        // Iterating over the tasks in ascending order of their latest start time
        while (jj < disj->_size && disj->_est[i] + disj->_dur_min[i] > disj->_lct[j] - disj->_dur_min[j]) {
...
static void dprec_filter_lct_vilim(CPDisjunctive * disj, const ORUInt * idx_map_lct, ThetaTree * theta, const ORUInt tsize, bool * update)
{
    // Inititialise Theta tree
    initThetaTree(theta, tsize, MAXINT);
    ORInt jj = disj->_size - 1;
    ORUInt j = disj->_task_id_ect[jj];
//    printf("+++++++\n");
    // Outer loop:
    // Iterating over the tasks in descending order of their latest start time
    for (ORInt ii = disj->_size - 1; ii >= 0; ii--) {
...
//
static void nfnl_filter_est_and_lct_vilim(CPDisjunctive * disj, const ORUInt * idx_map_est, const ORUInt * idx_map_lct, ThetaTree * theta, const ORUInt tsize, bool * update)
{
    nfnl_filter_est_vilim(disj, idx_map_lct, theta, tsize, update);
    nfnl_filter_lct_vilim(disj, idx_map_est, theta, tsize, update);
    if (update) updateBounds(disj);
}

static void nfnl_filter_est_vilim(CPDisjunctive * disj, const ORUInt * idx_map_lct, ThetaTree * theta, const ORUInt tsize, bool * update)
{
    ...
    for (ORInt ii = disj->_size - 1; ii >= 0; ii--) {
        ...
        assert(jj < (ORInt) (disj->_size - 1));
        assert(0 <= jLastInserted && jLastInserted < disj->_size);
        // Task 'i' is in the tree
        insertThetaNodeAtIdxLst(theta, tsize, idx_map_lct[i], 0, MAXINT);
...
static void nfnl_filter_lct_vilim(CPDisjunctive * disj, const ORUInt * idx_map_est, ThetaTree * theta, const ORUInt tsize, bool * update)
{
    // Inititialise Theta tree
    initThetaTree(theta, tsize, MININT);
    ORInt jj = 0;
    ...
    for (ORInt ii = 0; ii < disj->_size; ii++) {
        ...
        // Inner loop:
        // Iterating over the tasks in ascending order of their latest start time
        while (jj < disj->_size && disj->_lct[i] > disj->_lct[j] - disj->_dur_min[j]) {
            if (disj->_dur_min > 0) {
                // Checking for a new bound update of task 'j'
        ...
        assert(jj > 0);
        assert(0 <= jLastInserted && jLastInserted < (ORInt) disj->_size);
        // Task 'i' is in the tree
        insertThetaNodeAtIdxEct(theta, tsize, idx_map_est[i], 0, MININT);
...
//  NOTE: Tasks with a minimal duration of zero will be ignored.
//
static void ef_filter_est_and_lct_vilim(CPDisjunctive * disj, const ORUInt * idx_map_est, const ORUInt * idx_map_lct, ThetaTree * theta, LambdaTree * lambda, const ORUInt tsize, const ORUInt tdepth, bool * update)
{
    ef_filter_est_vilim(disj, idx_map_est, theta, lambda, tsize, tdepth, update);
    ef_filter_lct_vilim(disj, idx_map_lct, theta, lambda, tsize, tdepth, update);
    if (update) updateBounds(disj);
}

static void ef_filter_est_vilim(CPDisjunctive * disj, const ORUInt * idx_map_est, ThetaTree * theta, LambdaTree * lambda, const ORUInt tsize, const ORUInt tdepth, bool * update)
{
...
static void ef_filter_lct_vilim(CPDisjunctive * disj, const ORUInt * idx_map_lct, ThetaTree * theta, LambdaTree * lambda, const ORUInt tsize, const ORUInt tdepth, bool * update)
{
    // Initialise Theta-Lambda tree with (T, {})
    initThetaLambdaTreeWithLst(disj, idx_map_lct, theta, lambda, tsize);
    ORInt jj = 0;
...
static inline ORBool isUnfixed(CPDisjunctive * disj, const ORInt i)
{
    ...
    // scheduled.
static ORInt getGlobalSlack(CPDisjunctive * disj)
{
...
static ORInt getLocalSlack(CPDisjunctive * disj)
{
    ...
    disj->_task_id_lct   = alloca(disj->_size * sizeof(ORInt ));
    ...
    for (ORInt t = 0; t < disj->_size; t++) {
        ...
        disj->_lct    [t] = disj->_start0[t].max + disj->_dur0[t].max;
        disj->_dur_min[t] = disj->_dur0  [t].min;
        disj->_task_id_est[t] = t;
        disj->_task_id_lct[t] = t;
    }
    ...
    qsort_r(disj->_task_id_lct, disj->_size, sizeof(ORInt), disj, (int(*)(void*, const void*, const void*)) &sortDisjLctAsc);

    ORInt localSlack = MAXINT;
    ORInt len_min = 0;
    ORInt jjPrev = 0;
    ORInt jjLast = disj->_size - 1;
    
    for (ORInt jj = disj->_size - 1; jj >= 0; jj--) {
...
static void doPropagation(CPDisjunctive * disj) {
    ...
    disj->_task_id_est   = alloca(disj->_size * sizeof(ORInt ));
    disj->_task_id_ect   = alloca(disj->_size * sizeof(ORInt ));
    disj->_task_id_lst   = alloca(disj->_size * sizeof(ORInt ));
    disj->_task_id_lct   = alloca(disj->_size * sizeof(ORInt ));
    ...
    const ORUInt tsize = 2 * disj->_size - 1;
    ...
    for (ORInt t = 0; t < disj->_size; t++) {
        ...
        disj->_lct    [t] = disj->_start0[t].max + disj->_dur0[t].max;
        disj->_dur_min[t] = disj->_dur0  [t].min;
        disj->_new_est[t] = disj->_est[t];
        disj->_new_lct[t] = disj->_lct[t];
        disj->_task_id_est[t] = t;
        disj->_task_id_ect[t] = t;
        disj->_task_id_lst[t] = t;
        disj->_task_id_lct[t] = t;
        idx_map_est[t] = t;
        idx_map_lct[t] = t;
    }
    ...
    do {
        ...
        initIndexMap(disj->_task_id_est, idx_map_est, disj->_size, tsize, tdepth);
        
        // Consistency check
        ef_overload_check_vilim(disj, idx_map_est, theta, tsize);

        // Further initialisations needed for the filtering algorithm
        initIndexMap(disj->_task_id_lct, idx_map_lct, disj->_size, tsize, tdepth);
        qsort_r(disj->_task_id_ect, disj->_size, sizeof(ORInt), disj, (int(*)(void*, const void*, const void*)) &sortDisjEctAsc);
        qsort_r(disj->_task_id_lst, disj->_size, sizeof(ORInt), disj, (int(*)(void*, const void*, const void*)) &sortDisjLstAsc);
        
        // Detectable precedences
        if (disj->_dprec) {
            dprec_filter_est_and_lct_vilim(disj, idx_map_est, idx_map_lct, theta, tsize, & update);
        }
        // Not-first/not-last
        if (!update && disj->_nfnl) {
            nfnl_filter_est_and_lct_vilim(disj, idx_map_est, idx_map_lct, theta, tsize, & update);
        }
        // Edge-finding
        if (!update && disj->_ef) {
            ef_filter_est_and_lct_vilim(disj, idx_map_est, idx_map_lct, theta, lambda, tsize, tdepth, & update);
        }
    } while (disj->_idempotent && update);
    
    // Updating the global slack
    const ORInt globalSlack = getGlobalSlack(disj);
