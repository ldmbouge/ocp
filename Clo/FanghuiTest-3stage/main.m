...
ORInt optval;
struct AESSol aesSolutions[3000];
int aesSolutionsCount = 0;
struct AESSol finalSol;
...
bool verify(id<ORModel> model);
ORInt findOpt(id<ORModel> model, val);
void SolveAll(id<ORModel> model, ORInt objval);
...
int main(int argc, const char * argv[]) {
  cmd = [ORCmdLineArgs newWith:argc argv:argv];
  ORInt kKeys = 0;
  //hwrounds = [cmd size];
  ...
  currentErrors = [ORFactory intVar:model bounds:[ORFactory intRange:model low:0 up:116*2]];
  miniVar = [ORFactory intVar:model bounds:[ORFactory intRange:model low:0 up:116*2]];
  [model add: [Sum(model,j,[ORFactory intRange:model low:0 up:116-1],[errorPtr[2*j] plus: errorPtr[2*j + 1]]) eq: miniVar]];
  [model add: [Sum(model,j,[ORFactory intRange:model low:0 up:116-1],[errorPtr[2*j] plus: errorPtr[2*j + 1]]) eq: currentErrors]];

  id<ORModel> baseModel = [model copy];
  id<ORModel> solveallModel = [model copy];

  for(int i = 1; i < 10; i++)
    sideChannelCon(i);

  id<ORModel> fullModel = [model copy];
  ...
  R = [[ORIntRangeI alloc] initORIntRangeI:0 up:15];
  //id<ORIntRange> R2 = [[ORIntRangeI alloc] initORIntRangeI:0 up:47];
  o = (id)[CPFactory bitVarArray:model range: R];
  m = (id)[CPFactory bitVarArray:model range: R];
  /*
  ...
  */
  ...
  do{
    //firstrun = false;
    curr = findOpt(baseModel, curr);
    //curr = 10;
    NSLog(@"currObj = %d", curr);
    finalSol.objval = -1;
    SolveAll(solveallModel, curr);
    NSLog(@"# of Sol: %d", aesSolutionsCount);
  }
  while(!verify(fullModel));


  ORLong searchStop = [ORRuntimeMonitor wctime];
  ORDouble elapsed = ((ORDouble)searchStop - searchStart) / 1000.0;
  printf("FinishTime(s): %f\n",elapsed);
  NSLog(@"Solution:");
  NSLog(@"Objective: %d", finalSol.objval);
  for(int i = 0; i < 16; i++){
    printf(" %d", finalSol.keys[i]);
  }

  return 0;
}

ORInt findOpt(id<ORModel> model, ORInt val){
  ...
  [cp solve:^(){
	...
	if(currentobj > [cp intValue:miniVar]){
	  ...
	  currentobj = [[cp objectiveValue] intValue];
	  optval = currentobj;

	  for(int i = 0; i < 16; i++)
	    printf("%d ", checkKey([cp stringValue:keys[0][i]]));
	  printf("\n");
	}
      }
    }];
...
void SolveAll(id<ORModel> model, ORInt objval){
  ...
  [cp solveAll:^(){
      NSLog(@"Solveall Search Start");
      NSLog(@"CurrentErrors: %d", [cp min:currentErrors]);
      bool nfixed = true;
      for(int i = 0; i < 16; i++){
	if(!([cp domsize: m[i]] > 0)){
	  nfixed = false;
	  ...
	  //return ((([labeled[legpos[i]] value] == 10000) + ([labeled[mc_pos[legpos[i]][1]] value] == 10000)) << 10) + valuePairCount[i];
		  ...
		  [cp tryall:S suchThat:^ORBool(ORInt k) {
			...
			&& (rightValue == -1 || rightValue == valuePair[s][k].y);// [labeled[right] value] == valuePair[s][k].y)
		      ...
		      //&& (currentobj >= ([accumscore value] + valuePair[s][k].score - totalContribution)); //solve all
		    } orderedBy:^ORDouble(ORInt k) {
		      //return valuePair[s][k].score;
		      return (valuePair[s][k].score << 10) +  valuePair[s][k].hits;
		    }
	...
	if(objval == [cp intValue:miniVar]){
	  ...
	  */
	  for(int i = 0; i < 16; i++){
	    temp.keys[i] = checkKey([cp stringValue:keys[0][i]]);
	  }
	  aesSolutions[aesSolutionsCount++] = temp;
	}
      }
    }];
...
bool verify(id<ORModel> model){
  ...
  [cp solve:^(){
      ...
      [cp forall:R suchThat:^ORBool(ORInt i) {return true;/* && i % 2 == 0*/;} orderedBy:^ORInt(ORInt i) {
	  return true;
	  //return ((![cp bound: branchVarPtr[legpos[i]]]+ ![cp bound: branchVarPtr[mc_pos[legpos[i]][1]]]) << 10) + valuePairCount[i];
	  //return ((([labeled[legpos[i]] value] == 10000) + ([labeled[mc_pos[legpos[i]][1]] value] == 10000)) << 10) + valuePairCount[i];
	}
	      do:^(ORInt s) {
		  id<ORIntRange> S = [ORFactory intRange:cp low:0 up:(aesSolutionsCount - 1)];
		  ...
		  [cp tryall:S suchThat:^ORBool(ORInt k) {
			  ...
			  in:^(ORInt k) {
			    ...
			    struct AESSol candidate = aesSolutions[k];
			    for(int i = 0; i < 16; i++)
			      printf("%d ", candidate.keys[i]);
			    printf("\n");
				...
				for(int nbit = 0; nbit < 8; nbit++){
				  for(int i = 0; i < 16; i++){
				    BOOL val = (candidate.keys[i] >> count) & 1;
				    [cp labelBV:keys[0][i] at:nbit with:val]; // if the bit is already fixed, attempting to fix it to something else fails.
				  }
				  count++;
				}
			      }];
	...
	*/
	struct AESSol temp;
	temp.objval = [cp intValue:miniVar];
	for(int i = 0; i < 16; i++){
	  temp.keys[i] = checkKey([cp stringValue:keys[0][i]]);
	}
	finalSol = temp;
...
void readFile(FILE *f){
  ...
  while (fscanf(f, " %1023s", x) == 1) {
    if(count < 10);
      ...
      Plaintext[count-10] = atoi(x);
    }
    else if(count < 42){
      expect_key[(count-10) % 16] = atoi(x);
    }
    else if(count < 58){
      Ciphertext[(count-10) % 16] = atoi(x);
