...
-(id<ORBitVar>(*)[])round2:(id<ORBitVar>[])h x:(id<ORBitVar>[]) x
   {
      ...
      /*
       A = shuffle2(A,B,C,D,3,3,x);//[A B C D 3  3]
       D = shuffle2(D,A,B,C,7,5,x);//[D A B C 7  5]
       C = shuffle2(C,D,A,B,11,9,x);//[C D A B 11 9]
       B = shuffle2(B,C,D,A,15,13,x);//[B C D A 15 13]
       */
