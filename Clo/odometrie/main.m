...
//         id<ORFloatVar> diff2 = [ORFactory floatVar:model name:@"diff2"];
         /* Input */
         ...
         [model add:[delta_d eq:[[delta_dl plus:delta_dr] mul:expr_3]]];
...
//         TMP_6 = (0.1 * (0.5 * (9.691813336318980 - (12.34 * sl)))) ;
         ...
         for (ORUInt n = 0; n < NBLOOPS; n++) {
            ...
            [model add:[arg[n] eq:[theta[n] plus:[delta_theta mul:expr_3]]]];
            [model add:[cos[n] eq:[[expr_1 sub:[[arg[n] mul:arg[n]] mul:expr_3]] plus:[[[[arg[n] mul:arg[n]] mul:arg[n]] mul:arg[n]] div:@(24.0f)]]]];
...
//            TMP_23 = ((theta + (((9.691813336318980 - (sl * 12.34)) * 0.1) * 0.5)) * (theta + (((9.691813336318980 - (sl * 12.34)) * 0.1) * 0.5))) ;
            [model add:[TMP_23[n] eq:[[theta_opt[n] plus:[[[expr_4 sub:[sl mul: c]] mul: expr_2] mul: expr_3]] mul: [theta_opt[n] plus: [[[expr_4 sub: [sl mul: c]] mul: expr_2] mul: expr_3]]]]];
...
//            TMP_25 = ((theta + TMP_6)*(theta + TMP_6))*(theta + (((9.691813336318980 - (sl * 12.34)) * 0.1) * 0.5)) ;
            [model add:[TMP_25[n] eq:[[[theta_opt[n] plus:TMP_6] mul:[theta_opt[n] plus:TMP_6]] mul:[theta_opt[n] plus:[[[expr_4 sub:[sl mul:c]] mul:expr_2] mul:expr_3]]]]];
            [model add:[TMP_26[n] eq:[theta_opt[n] plus:TMP_6]]];

            [model add:[x_opt[n+1] eq:[[expr_3 mul:[[[expr_1 sub:[TMP_23[n] mul:expr_3]] plus:[[TMP_25[n] mul:TMP_26[n]] div:@(24.0f)]] mul:[[c mul: sl] plus:expr_4]]] plus:x_opt[n]]]];
            
//            x_opt = ((0.5f * (((1.0f - (TMP_23 * 0.5f)) + ((TMP_25 * TMP_26) / 24.0f)) * ((c * sl) + c2))) + x_opt) ;
            
            [model add:[TMP_27[n] eq:[[TMP_26[n] mul: TMP_26[n]] mul:[theta_opt[n] plus:[[[expr_4 sub:[sl mul:c]] mul:expr_2] mul: expr_3]]]]];
            
            //            TMP_27 = ((TMP_26 * TMP_26) * (theta_opt + (((c2 - (sl * c)) * 0.1f) * 0.5f))) ;
          
            [model add:[TMP_29[n] eq:[[[TMP_26[n] mul: TMP_26[n]] mul: TMP_26[n]] mul:[theta_opt[n] plus:[[[expr_4 sub:[sl mul:c]] mul:expr_2] mul:expr_3]]]]];
            //            TMP_29 = (((TMP_26 * TMP_26) * TMP_26) * (theta_opt + (((c2 - (sl * c)) * 0.1f) * 0.5f))) ;
            
            
            [model add:[y_opt[n+1] eq:[[[expr_4 plus:[c mul: sl]] mul:[[[TMP_26[n] sub: [TMP_27[n] div:@(6.0f)]] plus:[[TMP_29[n] mul: TMP_26[n]] div: @(120.0f)]] mul:expr_3]] plus: y_opt[n]]]] ;
            //            y_opt = (((c2 + (c * sl)) * (((TMP_26 - (TMP_27 / 6.0f)) + ((TMP_29 * TMP_26) / 120.0f)) * 0.5f)) + y_opt) ;
         ...
         [model add:[diff eq:[y_opt[NBLOOPS] sub:y[NBLOOPS]]]];
         [model add:[[diff mul:diff] eq:@(0.0f)]];
//         [model add:[diff2 eq:[x_opt[NBLOOPS] sub:x[NBLOOPS]]]];
//         [model add:[diff gt:@(0.0f)]];
//         [model add:[diff2 eq:[diff mul:diff]]];
//         [model add:[diff2 gt:@(0.0f)]];
